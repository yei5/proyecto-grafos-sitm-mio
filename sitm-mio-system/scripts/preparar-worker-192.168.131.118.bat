@echo off
REM Script para preparar el host Worker 3 (192.168.131.118 - Node4)
REM Usuario: swarch
...
echo mkdir -p ~/sitm-mio-distribuido/datagram-worker-service/build/libs > %TEMP%\create_dirs.sh
echo mkdir -p ~/sitm-mio-distribuido/config >> %TEMP%\create_dirs.sh
echo mkdir -p ~/sitm-mio-distribuido/db/node >> %TEMP%\create_dirs.sh
echo echo    [OK] Directorios creados >> %TEMP%\create_dirs.sh
